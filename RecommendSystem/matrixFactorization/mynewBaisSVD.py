...
class  BaisSVD:
    def __init__(self,X,k=20):  
        ...
        for i in range(self.X.shape[0]):  
            ...
            mid=self.X[i][1]  
            rat=self.X[i][2]  
            self.movie_user.setdefault(mid,{})#user_movie:record all users who have rated for movie mid
            self.user_movie.setdefault(uid,{})#movie_user:record all movies which have been rated by user uid
            self.movie_user[mid][uid]=rat  
            self.user_movie[uid][mid]=rat  
            self.bi.setdefault(mid,0)#init bi and qi =0
            self.bu.setdefault(uid,0)  
            self.qi.setdefault(mid,random((self.k,1))/10*(np.sqrt(self.k)))#randomly generate array k*1,value:0~1/10*sqrt(k)
    ...
    def pred(self,uid,mid):  
        ...
        if np.all(self.qi[mid]== None):
            self.qi[mid]=np.zeros((self.k,1))
        if np.all(self.pu[uid]==None):
    ...
    def train(self,steps=20,gamma=0.04,Lambda=0.15):  
        for step in range(steps):  
            ...
            rmse_sum=0.0  
            kk=np.random.permutation(self.X.shape[0]) #randomly arrange all numbers from 0 to X.shape[0]-1
            #kk is a list,contain 0~m-1,randomly
            #in oder to train randomly
...
if __name__ == "__main__":
    train_data_path='D:/PycharmProjects/RecommendSystem/matrixFactorization/trainingSet.csv'
    test_data_path='D:/PycharmProjects/RecommendSystem/matrixFactorization/testingSet.csv'
    train_data=[]
    test_data= []
    ...
    with open(train_data_path, 'r') as f:
        ...
        for line in reader:
            if line[0]!='userId':
                num_term = []
                num_term.append(int(line[0]))
                num_term.append(int(line[1]))
                num_term.append(float(line[2]))
                train_data.append(num_term)
        train = train_data[1:]
