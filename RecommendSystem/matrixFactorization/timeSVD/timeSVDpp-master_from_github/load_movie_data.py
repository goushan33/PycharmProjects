...
class loadMovieData:
    def read_training_data(self, path):
         with open(path, 'rb') as f:
            ...
            for line in f:
                ...
                b += 1
            print('#pos_events = %s'% b)
            min_timestamp = min(timestamps)
            max_timestamp = max(timestamps)

            print('max item id = %s'%max(max_item))
    ...
    def main(self):
        matrix, userItems, itemUsers, min_timestamp, max_timestamp = self.read_training_data("D:\\PycharmProjects\\RecommendSystem\\matrixFactorization\\timeSVD\\timeSVDpp-master_from_github\\ml-100k\\u1.base")
